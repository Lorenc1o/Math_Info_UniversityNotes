...
public class NCShell {
	...
	private String[] readChatCommandFromStdIn(NCConnector ncclient) {
		...
		while (true) {
			...
			case NCCommands.COM_PRIVATE_SEND:
				...
				while (st.hasMoreTokens()) {
					mes.append(st.nextToken() + " ");
				}
				if (!mes.toString().equals(""))
					vargs.add(mes.toString());
				break;
	...
	private boolean validateCommandArguments(String[] args) {
		...
		case NCCommands.COM_CREATE_ROOM:
			if (args.length == 0 || args.length > 1) {
				System.out.println("Correct use: createroom <roomname>");
				return false;
			}
			break;
		...
		case NCCommands.COM_PRIVATE_SEND:
			if (args.length < 2) {
				System.out.println("Correct use: privatesend <user> <message>");
				return false;
			}
			break;
		case NCCommands.COM_RENAME:
			if (args.length == 0 || args.length > 1) {
				System.out.println("Correct use: rename <roomname>");
				return false;
			}
			break;
		case NCCommands.COM_ASCENDER:
		...
		case NCCommands.COM_EXPEL:
			if (args.length == 0 || args.length > 1) {
				System.out.println("Correct use: expel <user>");
				return false;
			}
			break;
		default:
